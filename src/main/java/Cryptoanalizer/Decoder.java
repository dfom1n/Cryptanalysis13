            ...
            while ((strIn = bufferedReader.readLine()) != null) {
                ...
                for (int i = 0; i < listIn.size(); i++) {
                    if (ARR_LOWER_LETTER.contains(listIn.get(i))) {
                        ...
                        int numberLetter;
                        if ((number - KEY_CRYPT) >= 0 && (number - KEY_CRYPT) < ARR_LOWER_LETTER.size()) {
                            numberLetter = number - KEY_CRYPT;
                        } else {
                            numberLetter = Math.abs(ARR_LOWER_LETTER.size() + (number - KEY_CRYPT));
                        }
...
//                    String strOut = listOut.toString();
